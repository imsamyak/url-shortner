<template>
  <div class="surface-card overflow-hidden">
    <div class="flex flex-col justify-between gap-4 border-b border-slate-100 px-6 py-6 sm:flex-row sm:items-end sm:px-8">
      ...
      <span class="flex w-fit items-center gap-2 rounded-xl bg-brand-50 px-3 py-2 text-xs font-bold text-brand-700"><span class="h-2 w-2 animate-pulse rounded-full bg-emerald-500" /> Select a service</span>
    </div>

    <div class="overflow-x-auto">
      <div class="min-w-[1180px] bg-[radial-gradient(circle_at_20%_10%,rgba(59,130,246,.08),transparent_30%),radial-gradient(circle_at_85%_25%,rgba(34,211,238,.08),transparent_28%)] p-8">
        ...
        <div class="grid grid-cols-[100px_36px_145px_36px_1fr_36px_145px] items-stretch">
          ...
          <div class="rounded-[1.75rem] border-2 border-dashed border-brand-300 bg-brand-50/30 p-4">
            <div class="mb-4 flex items-center justify-between"><div class="flex items-center gap-2"><span class="flex h-7 w-7 items-center justify-center rounded-lg bg-brand-100 text-xs font-black text-brand-700">V</span><p class="text-xs font-black uppercase tracking-[0.13em] text-brand-700">Shared VPC · 10.20.0.0/16</p></div><span class="text-[10px] font-bold text-brand-500">2 availability zones</span></div>
            <div class="grid grid-cols-[1fr_30px_1fr_30px_1fr_30px_1fr] items-stretch">
              <div class="rounded-2xl border border-cyan-200 bg-cyan-50/80 p-3">
                ...
                <button type="button" class="mt-2 w-full rounded-xl border border-cyan-200 bg-white p-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" @click="emit('explore', 'alb')"><span class="text-[10px] font-bold text-slate-400">WAF protected</span><span class="mt-1 block text-sm font-black text-slate-950">Public ALB</span><span class="mt-1 block text-[10px] text-slate-500">Listener :80</span></button>
              </div>
              ...
              <div class="rounded-2xl border border-violet-200 bg-violet-50/80 p-3"><p class="text-[9px] font-black uppercase tracking-[0.12em] text-violet-700">Private endpoint</p><button type="button" class="mt-2 w-full rounded-xl border border-violet-200 bg-white p-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" @click="emit('explore', 'express')"><span class="text-[10px] font-bold text-slate-400">WAF protected</span><span class="mt-1 block text-sm font-black text-slate-950">Internal ALB</span><span class="mt-1 block text-[10px] text-slate-500">Nuxt ingress only</span></button></div>
          ...
          <DiagramArrow label="SDK" />
          <DiagramNode eyebrow="Data plane" title="DynamoDB" icon="D" tone="amber" description="PK/SK · GSI1 · TTL" interactive @click="emit('explore', 'dynamodb')" />
        </div>
        ...
        <div class="grid grid-cols-3 gap-5">
          <section class="rounded-3xl border border-blue-200 bg-blue-50/60 p-4">
            ...
            <div class="mt-4 flex items-center gap-2"><MiniNode icon="G" title="GitHub" /><span class="text-brand-400">→</span><MiniNode icon="P" title="2× Pipeline" /><span class="text-brand-400">→</span><MiniNode icon="R" title="2× ECR" /><span class="text-brand-400">→</span><MiniNode icon="C" title="CodeDeploy" /></div>
          ...
          <section class="rounded-3xl border border-violet-200 bg-violet-50/60 p-4">
            ...
            <div class="mt-4 flex items-center gap-3"><MiniNode icon="S" title="Secrets Manager" wide /><span class="text-violet-400">→</span><MiniNode icon="E" title="Express role" wide /></div>
          ...
          <section class="rounded-3xl border border-emerald-200 bg-emerald-50/60 p-4">
            ...
            <div class="mt-4 flex items-center gap-2"><MiniNode icon="L" title="App logs" /><span class="text-emerald-500">→</span><MiniNode icon="W" title="CloudWatch" /><span class="text-emerald-500">→</span><MiniNode icon="S" title="SNS alarms" /></div>
